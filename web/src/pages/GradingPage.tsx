...
const GROUPS: Group[] = [
  {
    ...
    items: [
      ...
      { k: "grade_check_empty_folders", label: "Empty folders", desc: "A folder with nothing at all under it fails the run (issue code EMPTY_FOLDER) — albums come from audio files, so an empty folder would otherwise be skipped silently. Hidden and app-state folders are ignored." },
      { k: "grade_check_expected_tracks", label: "Release tracklist manifest", desc: "An album whose folder has no .mlo_expected.json fails the run (issue code EXPECTED_TRACKS_MISSING). The manifest records the MusicBrainz release's own tracklist, which is the only way a partial import can name the tracks that never arrived; script 15 (Release tracklist) writes it, and the album page greys out the missing tracks from it." },
      ...
      { k: "grade_check_disc_naming", label: "Disc folder naming", desc: "Disc folders and rip sheets must follow the configured disc pattern (CD-1, CD-2 … by default). The check runs whatever auto-rename is set to — with it off, renaming them is a manual job (the issue text says so)." },
