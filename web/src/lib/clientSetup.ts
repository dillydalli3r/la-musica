...
/** This probe is a person waiting on a "Test" button, not a page load — three
 *  seconds is long enough for a LAN address to answer and short enough that a
 *  wrong one is a reply, not a hang. */
 ...
 *
 *  A shell bundles no backend of its own any more, so this is the only way a
 *  client learns where one runs — and the wizard's Next stays locked until an
 *  address has answered.
 *
