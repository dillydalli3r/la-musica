...
# Where the gato picture lives, in the order the layouts put it: a checkout
# keeps it in web/public, and the RELEASE ZIP carries the BUILT frontend.
# web/public is not in that zip — looking there alone threw
# FileNotFoundError, so every installed copy silently drew the play-tile
# fallback below instead of the cat. Keep every layout the app ships in on
# this list; desktop/src-tauri/icons is what the Tauri bundle carries.
ICON_CANDIDATES = (
    ("web", "public", "icon.png"),
    ("web", "dist", "icon.png"),
...
def _icon_source():
    """The first app icon this layout actually has, or None."""
...
def make_icon():
    """The app icon (the gato picture, regenerated by tools/make_icons.py)
    center-cropped with rounded corners for the tray, falling back to a
    drawn dark play-tile only when no copy of it can be found."""
    size = 64
    # PIL first, OUTSIDE the try: the fallback below draws with these names,
    # and an import placed after the icon lookup left them unbound there.
    from PIL import Image, ImageDraw
    try:
        path = _icon_source()
        if not path:
            raise FileNotFoundError("no icon.png in this layout")
