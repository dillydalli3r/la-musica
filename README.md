...
**v4.1.10** — a self-hosted app that *manages, optimizes, audits, grades and plays* your music library, from a browser, a desktop window or a phone.
...
- **Browsing your OWN account is answered from your own share, and says so.** That browse is the one the peer network cannot serve from inside your network — the Soulseek server hands every client the address it published for the account (your own public one) and a router without NAT loopback refuses that dial — so the app reads the index slskd serves (the very tree a peer gets), marks the answer `local: true` and shows why. Nothing is walked from disk and nothing is invented; every other username still goes over the peer network.
