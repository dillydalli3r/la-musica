...
**v4.0.0** — a self-hosted app that *manages, optimizes, audits, grades and plays* your music library, from a browser, a desktop window or a phone.
...
- Lyrics come from the `.lrc` beside the file and/or its `LYRICS` tag; a stamp before the file's start clamps at `[00:00.00]`. Every surface that shows lyrics also says **which kind** they are — **Synced** (the `.lrc` or embedded lyrics carry timestamps) or **Plain** — from the file's own stored truth (`lyrics_kind` in the payload); with `lyrics_allow_plain` off (the shipped default) a plain lyric is shown as the failing state it is, with the reason naming the setting, and a track with no lyrics shows no kind at all.
