...
**v4.1.11** — a self-hosted app that *manages, optimizes, audits, grades and plays* your music library, from a browser, a desktop window or a phone.
...
- **Is the port open?** *Test port* (`GET /api/soulseek/port-check`) answers with seven rows that each say what they prove: a real TCP connection, a bind test, whether the host publishes the very port the daemon holds (in a container — measured from inside, with the app's own published port as the control), the gateway's own mapping entry, the LAN/WAN address shape (CGNAT named as such, and the one 100.64.0.0/10 address that is a tunnel carrying this host's traffic told apart from the carrier's by the routes, never by the address — a bare-metal install gets the egress check that a container cannot make), a public-address self-connect (refused ⇒ unknown), the address the NETWORK publishes for the account (slskd browses its own username: a port that differs from the listen port is a failure carrying its remedy, a matching address that refuses is unknown and spells out what a client on the same network gets — measured on the owner's own install: `peers are told 216.212.53.255:50000`, and an outside peer read 15 directories / 165 files in 0.09 s) and slskd's signed-in state.
- **Browsing your OWN account is answered from your own share, and says so.** That browse is the one the peer network cannot serve from inside your network — the Soulseek server hands every client the address it published for the account (your own public one) and a router without NAT loopback refuses that dial — so the app reads the index slskd serves (the very tree a peer gets), marks the answer `local: true` and shows why. The sharing card opens it with **Browse my share**, so what a peer is served is one press away — no router setting, no host setup. Nothing is walked from disk and nothing is invented; every other username still goes over the peer network.
...
### Soulseek in Docker: the three links that have to line up

Sharing is not the web UI. Another user reaches your files by opening a connection **back** to the listen port (default 50000, `soulseek_listen_port`), and that one TCP port has to survive three hops before it does — **TCP only, no UDP and no second (obfuscated) port**, because slskd implements no obfuscated route and a peer that tries one falls back to this port. The three links go in this order, and *Test port* (`GET /api/soulseek/port-check`) plus the sharing card's own rows name the one that failed rather than blaming the rest.

1. **The container publishes it.** `docker-compose.yml` publishes `${MLO_SOULSEEK_LISTEN_PORT:-50000}` as the same number on both sides, and the *Published by the host* row MEASURES it from inside: a published port is accepted on Docker's gateway, the app's own 8000 is the control, so a missing publish line is only called that when this Docker demonstrably hands published ports back. A line that is missing or on another number is reported as itself — the audit's `listen_unpublished`, naming the exact `ports:` line and the pin — instead of as a router that will not answer.
2. **The router forwards it to the HOST.** A router cannot forward to a container address: 172.18.0.x is Docker's bridge on the host and nothing on your LAN can dial it, so the forward names the **HOST's** LAN address and the publish line carries the port the rest of the way in. Automatic opening (`soulseek_upnp`, ON) is the app's own UPnP-then-NAT-PMP request, and from inside a container it never leaves Docker's bridge — name the router in **Router IP for port mapping** (`soulseek_router_ip`) or make the forward by hand. A container cannot read the host's port list or its routing table: the app measures the publish line from inside, and asks you to check the rest on the host.
3. **The HOST's traffic actually leaves by the ISP line.** This is the one that reads green everywhere — published, forwarded, slskd logged in, share indexed, a peer even fetching your file count — and still cannot work, because a peer's *Browse* is a connection BACK and the address they dial is the one the Soulseek server learned from your **login connection**. If the host routes its traffic through a VPN or a Tailscale exit node, that address is the VPN's egress and no forward on your router can serve it (a live install exactly: the host's egress was an exit node's address while the router's own WAN was a different, public one, so the share's size showed on other clients and opening it never worked). **Check it on the HOST**: compare your own egress (any "what is my IP" page, or `curl https://api.ipify.org`) with the WAN address your router's admin page shows — *Test port*'s *Addresses* row reports that same number for the router when a gateway read answered, and the host's egress is the one number no read from inside the container can supply. When the two differ, turn the exit node/VPN off for the host while sharing, or split-route it so this app's traffic leaves by the ISP line. A bare-metal install has this measured for it — the *Addresses* row of *Test port* fails there, with the addresses it read, when the internet-bound route is a tunnel (and says something different, and true, when the 100.64.0.0/10 address is the carrier's instead: that one is the ISP's to fix).

**And a client on YOUR OWN network cannot prove any of it.** A Soulseek client on the same LAN is handed your public address by the Soulseek server and dials it from *inside*; a router without NAT hairpinning (most of them, and this app's own `Self-connect` row says so) refuses exactly that connection while the port is wide open to everyone else — so the client sits on *Requesting file list…* forever and the share is fine. slskd browsing your own username fails the same way. The test that means something starts on another network: a phone on cellular with a Soulseek client, another user, or an online TCP check of the WAN address and the listen port (the app prints the router's WAN address in *Test port* → *Addresses*). If you want the local client to work too, that is the router's *NAT loopback* / *reflection* option, not anything on this side.
...
| `GET /api/soulseek/port-check` | the listen port's own check (a TCP connect, a bind test, whether the host publishes that very port — read from inside a container, with the app's own published port as the control — the router's mapping entry, the address shape: the LAN/WAN pair, a CGNAT, or a 100.64.0.0/10 tunnel carrying this host's traffic, told apart by the routes — a public-address self-connect, slskd's state) |
