...
- **Which edition is fetched** is `mlo/release_choice.py`, the same policy the release-group page shows: official editions first, then the configured medium order, then the earliest date (and a comment-free title over a commented one when everything else ties). Disc streams are preferred (`prefer_disc_streams`), and a disc structure beside a re-encode is remuxed as the disc's own title.
